...
export const esignApi = {
  ...
  prepareSigning: async (assetId: number): Promise<InitiateSigningDto> => {
    const response = await apiClient.get(`/esign/envelopes/prepare/${assetId}`);
  ...
  getEnvelopeStatus: async (envelopeId: string) => {
    const response = await apiClient.get(`/esign/envelopes/${envelopeId}/status`);
    return response.data;
  },
   ...
   */
  getEnvelopeActivities: async (envelopeId: string): Promise<ActivityStatus[]> => {
    const response = await apiClient.get(`/esign/envelopes/${envelopeId}/activities`);
    return response.data;
  },
};
