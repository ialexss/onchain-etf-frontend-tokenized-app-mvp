...
export default function DocumentDetailPage() {
	...
	const signMutation = useMutation({
		mutationFn: ({
			...
			documentsApi.signDocument(documentId, {
				signerEmail: email,
				signerType,
			}),
		onSuccess: () => {
		...
		onError: (error: any) => {
			toast.error(
				error.response?.data?.message || "Error al firmar el documento"
			);
		},
	});
	...
	const canSign = () => {
		...
		if (document.type === "CD" || document.type === "BP") {
			if (orgType === "WAREHOUSE" && !document.signedByWarehouse)
				return true;
	...
	if (!document) {
		return (
			<div className="flex flex-col items-center justify-center h-64">
				<p className="text-zinc-500 mb-4">Documento no encontrado</p>
