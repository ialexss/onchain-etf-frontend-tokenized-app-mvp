...
export function DocumentViewer({ document, onSign, canSign }: DocumentViewerProps) {
  ...
  const getDocumentTypeName = () => {
    switch (document.type) {
      case 'CD': return 'Certificado de Depósito';
      case 'BP': return 'Bono de Prenda';
      case 'ENDORSEMENT': return 'Documento de Endoso';
  ...
  const isFullySigned = () => {
    if (document.type === 'ENDORSEMENT') {
      return document.signedByWarehouse && document.signedByClient && document.signedByBank;
    }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Versión {document.version}</p>
            </div>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">Almacén (WAREHOUSE)</span>
              {document.warehouseSignatureDate && (
                <span className="text-xs text-slate-500">
                  {new Date(document.warehouseSignatureDate).toLocaleDateString()}
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              {document.clientSignatureDate && (
                <span className="text-xs text-slate-500">
                  {new Date(document.clientSignatureDate).toLocaleDateString()}
                </span>
              )}
            </div>
            {document.type === 'ENDORSEMENT' && (
              <div className="flex items-center gap-2">
                ...
                {document.bankSignatureDate && (
                  <span className="text-xs text-slate-500">
                    {new Date(document.bankSignatureDate).toLocaleDateString()}
                  </span>
                )}
              </div>
            )}
